...
const startDragging = (elementType, event) => {
  ...
  if (event.target.closest('.resize-handle') || 
      ...
      event.target.closest('.timer-btn')) return
...
const startResizing = (elementType, direction, event) => {
  ...
  const currentSize = elementType === 'question' ? questionSize.value : optionsSize.value
  ...
  event.currentTarget.style.cursor = direction === 'right' ? 'ew-resize' : 
...
const onResize = (event) => {
  ...
  const deltaX = event.clientX - resizeStartData.x
  const deltaY = event.clientY - resizeStartData.y
  
  const minWidth = 200
  const minHeight = 100
  const maxWidth = window.innerWidth - 20  // Much more generous width
  const maxHeight = window.innerHeight - 50 // Much more generous height
...
const resetStyles = () => {
  ...
  questionSize.value = { width: 500, height: 250 }
  optionsSize.value = { width: 450, height: 320 }
}
    ...
    <div class="board-container" :class="{ 'fullscreen': isFullscreen, 'dark-mode': darkMode }">
      ...
      >
        ...
        <div class="resize-handle resize-handle-right" 
             @mousedown="startResizing('question', 'right', $event)"></div>
        <div class="resize-handle resize-handle-bottom" 
             @mousedown="startResizing('question', 'bottom', $event)"></div>
        <div class="resize-handle resize-handle-corner" 
             @mousedown="startResizing('question', 'corner', $event)"></div>
      ...
      >
        ...
        <!-- Resize handles -->
        <div class="resize-handle resize-handle-right" 
             @mousedown="startResizing('options', 'right', $event)"></div>
        <div class="resize-handle resize-handle-bottom" 
             @mousedown="startResizing('options', 'bottom', $event)"></div>
        <div class="resize-handle resize-handle-corner" 
             @mousedown="startResizing('options', 'corner', $event)"></div>
