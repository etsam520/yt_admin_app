...
import { ref } from 'vue';
import { useRouter, useRoute } from 'vue-router'; // Import useRouter for programmatic navigation
import InputText from 'primevue/inputtext'; // Assuming these are PrimeVue components
...
const email = ref('');
const password = ref('');
const checked = ref(false); // For "Remember me" functionality
const errorMessage = ref(''); // To display login errors
const router = useRouter(); // Initialize router for Vue 3 Composition API
...
const handleLogin = async () => {
    ...
    try {
        ...
        if (response.status === 200 || response.status === 201) {
            ...
            if (token) {
                await localStorage.setItem('auth_token', response.data.data.token);
            }
            if (user) {
                await localStorage.setItem('user_name', user.name);
                // You can also set user in your Pinia store here
            }
            // 🔥 Set token in Axios immediately
            apiClient.defaults.headers.common['Authorization'] = `Bearer ${token}`;
            localStorage.setItem('user', JSON.stringify(user)); // Store user data in localStorage

            // Fetch and update permissions in the store after successful login
            await permissionStore.fetchPermissions();

            const redirectTo = route.query.redirect || '/dashboard';
            await router.push(redirectTo); // Use replace instead of push to prevent back to login
        }
    } catch (error) {
        ...
        if (error.response) {
            ...
            errorMessage.value = 'Error setting up login request.';
        }
    }
};
</script>
            ...
            <div style="border-radius: 56px; padding: 0.3rem; background: linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 150, 243, 0) 30%)">
                <div class="w-full bg-surface-0 dark:bg-surface-900 py-20 px-8 sm:px-20" style="border-radius: 53px">
                    <div class="text-center mb-8">
                        ...
                        <img alt="logo" src="/demo/images/logo.png" class="mb-4 w-40 shrink-0 mx-auto" />
                        <div class="text-surface-900 dark:text-surface-0 text-3xl font-medium mb-4">Welcome to YT</div>
