...
async function fetchQuestions() {
    try {
        ...
        + '&qbt=teacher'); // Fetch NPQ Teacher questions
...
onMounted(async () => {
   ...
   await console.log('Settings loaded');  // 1️⃣ runs first
   console.log(settings.questions.displayLanguagePreference)
    await fetchQuestions();        
    await console.log('Questions loaded');  // 2️⃣ runs after getSettings finishes
    await fetchSubjects();     
    await console.log('Subjects loaded');  // 3️⃣ runs after fetchQuestions finishes
    await loadOrganizations();  
    await console.log('Organizations loaded');  // 4️⃣ runs after fetchSubjects finishes
})
                ...
                <div class="p-4 bg-gray-50">
                    <div>
                        <div class="mb-6">
                            ...
                            <div>
                                <div class="text-xl font-medium text-black dark:text-white" :class="selectedQuestions" v-if="slotProps.data.formattedQuestion.question.text?.en != null" v-html="renderLatex(slotProps.data.formattedQuestion.question.text.en)"></div>
                                <div class="text-xl font-medium text-black dark:text-white" :class="selectedQuestions" v-if="slotProps.data.formattedQuestion.question.text?.hi != null" v-html="renderLatex(slotProps.data.formattedQuestion.question.text.hi)"></div>
