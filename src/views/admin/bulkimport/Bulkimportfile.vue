  ...
  try {
    for (const item of files) {
      ...
      if (props.endpoint) {
        ...
        const res =await apiClient.post(props.endpoint, form, {
          headers: {
        ...
        if(res.status == 200){
            item.progress = 100
            item.status = 'done'
            emit('uploaded', item.file, res.data)
        } else {
            item.status = 'error'
            item.error = `Upload failed with status ${res.status}`
            emit('error', item.file, item.error)
            continue;
        }
