...
import QuestionPreview from '@/views/admin/streamContent/QuestionPreview.vue';
...
const canPreview = computed(() => {
    return questionData.value.question.text || questionData.value.answer || questionData.value.solution.text;
});
...
const hasUnsavedChanges = () => {
    return (
        questionData.value.question.text.hi ||
        questionData.value.question.text.en ||
        questionData.value.question.images.length > 0 ||
        questionData.value.options.some((opt) => opt.text.en || opt.text.hi || opt.images.length > 0) ||
        questionData.value.answer ||
        questionData.value.answer ||
        questionData.value.answer.images.length > 0 ||
        questionData.value.solution.text.en ||
        questionData.value.solution.text.hi ||
...
onMounted(() => {
    ...
    //   uploadedFiles.value = [
    //     ...questionData.value.question.images,
    //     ...questionData.value.options.flatMap(opt => opt.images),
    //     ...questionData.value.answer.images,
    //     ...questionData.value.solution.images
    //   ]
    // }
...
const addOption = () => {
    if (questionData.value.options.length < 6) {
        questionData.value.options.push({ text: { hi: '', en: '' }, images: [] });
    }
};
...
const preparePayload = () => {
    ...
    payload.meta = {
        ...props.questionMeta,
        organisationId: props.questionMeta?.organisationId || null,
        subjectId: props.questionMeta?.subjectId || null,
        chapterId: props.questionMeta?.chapterId || null,
        topicId: props.questionMeta?.topicId || null
    };
...
//mapping editable quesiton
const mapEditableQuestion = async () => {
    const _q = (await props.qData) || {};
    if (!_q || Object.keys(_q).length === 0) return;

    console.log('Editable Question Data:', _q);
    questionData.value = _q;
    editingId.value = _q.id;
    // if(props.eId) {
    //   editingId.value = props.eId
    //   questionData.value = props.qData
    //   uploadedFiles.value = [
...
<template>
    <Card class="p-4">
        ...
        <template #content>
            <!-- Question Section -->
            <div class="mb-6">
                <label class="block font-bold mb-2">Question:English</label>
                <Textarea v-model="questionData.question.text.en" rows="3" class="w-full mb-2" placeholder="Type your question here. Use $$...$$ for LaTeX." :disabled="isSaving" />
                <label class="block font-bold mb-2">Question:Hindi</label>
                <Textarea v-model="questionData.question.text.hi" rows="3" class="w-full mb-2" placeholder="Type your question here. Use $$...$$ for LaTeX." :disabled="isSaving" />
            ...
            <div class="mb-6">
                ...
                <div v-for="(option, index) in questionData.options" :key="index" class="mb-4">
                    ...
                    <Textarea v-model="questionData.options[index].text.en" rows="2" class="w-full mb-2" :placeholder="'Option ' + String.fromCharCode(65 + index)" :disabled="isSaving" />
                    <Textarea v-model="questionData.options[index].text.hi" rows="2" class="w-full mb-2" :placeholder="'Option ' + String.fromCharCode(65 + index)" :disabled="isSaving" />
                    <div class="p-4 bg-white shadow rounded-md w-fit">
                        <div class="flex gap-2 flex-wrap">
                            <div class="flex items-center gap-2 px-3 py-1 rounded-full border border-gray-300 bg-gray-100 hover:bg-gray-200 cursor-pointer transition">
                                <RadioButton v-model="questionData.answer" :inputId="'opiont' + String.fromCharCode(65 + index)" name="pizza" :value="String.fromCharCode(65 + index)" />
                                <label for="ingredient2" class="text-sm">Mark Answer</label>
                            </div>
                        </div>
                    </div>
            ...
            <!-- Solution Section -->
            <div class="mb-6">
                <label class="block font-bold mb-2">Solution:English</label>
                <Textarea v-model="questionData.solution.text.en" rows="3" class="w-full mb-2" placeholder="Detailed solution with LaTeX support" :disabled="isSaving" />
                <label class="block font-bold mb-2">Solution:Hindi</label>
                <Textarea v-model="questionData.solution.text.hi" rows="3" class="w-full mb-2" placeholder="Detailed solution with LaTeX support" :disabled="isSaving" />
            ...
            <div class="mb-6 grid grid-cols-2 gap-4">
                <div class="md:col-span-2">
                    ...
                    <Chips id="specialization" v-model="questionData.tags" separator="," class="w-full" />
                    <small class="text-xs text-gray-500">Separate with commas (e.g., SSC , CBSE)</small>
                </div>
            </div>
            ...
            <!-- Preview Section -->
            <Preview v-if="showPreviewSection" :question="previewData" :mode="previewMode" @close="showPreviewSection = false" @change-mode="previewMode = $event" />
